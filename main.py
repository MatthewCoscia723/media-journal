...
def print_entries(entries):
    if not entries:
        ...
        return
    header = f"{'ID':<4} {'Type':<7} {'Rating':<7} {'Date':<12} {'Title':<30} Notes"
    print("\n" + header)
    print("-" * len(header))
    for e in entries:
        print(f"{e.id:<4} {e.media_type:<7} {e.rating:<7} {e.date:<12} {e.title:<30} {e.notes}")
    print()


def prompt_media_type():
    while True:
        t = input("  Type (movie / tv / game): ").strip().lower()
        if t in VALID_TYPES:
            return t
        print("  Please enter: movie, tv, or game")
...
def prompt_date():
    while True:
        d = input("  Date watched/played (YYYY-MM-DD): ").strip()
        parts = d.split("-")
...
def do_add():
    ...
    entry = journal.add_entry(title, media_type, date, rating, notes)
