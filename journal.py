...
def add_entry(title: str, media_type: str, date: str, rating: int, notes: str = "") -> Entry:
    entries = storage.load_entries()
    new_id = max((e.id for e in entries), default=0) + 1
    entry = Entry(title=title, media_type=media_type, date=date, rating=rating, id=new_id, notes=notes)
